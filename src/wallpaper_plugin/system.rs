    ...
    for (entity, mut window, mut cache) in &mut changed_windows {
        ...
        if window.mode != cache.window.mode {
            let new_mode = match window.mode {
                ...
                // }
                // }
                WindowMode::Fullscreen(_) => Some(None),
                WindowMode::SizedFullscreen(_) => Some(None),
        ...
        if window.cursor_options.grab_mode != cache.window.cursor_options.grab_mode
            && winit_windows::attempt_grab(winit_window, window.cursor_options.grab_mode).is_err()
        {
        ...
        if window.position != cache.window.position {
            if let Some(position) = super::winit_window_position(
                &window.position,
                &window.resolution,
                &monitors,
                winit_window.primary_monitor(),
